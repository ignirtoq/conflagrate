...
@pytest.mark.asyncio
async def test_execute_node_no_next_node(
        node, dependency_cache, mock_branch_tracker
):
    with mock.patch('conflagrate.engine.get_dependencies',
        ...
        await execute_node(node, mock_branch_tracker, dependency_cache)
...
@pytest.mark.asyncio
async def test_execute_node_one_next_node(
        node, dependency_cache, mock_branch_tracker
):
    node.edges = [node]
    with mock.patch('conflagrate.engine.get_dependencies',
...
@pytest.mark.asyncio
async def test_execute_node_two_next_nodes(
        node, dependency_cache, mock_branch_tracker
):
